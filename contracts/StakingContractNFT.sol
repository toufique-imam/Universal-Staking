    ...
    IERC721Receiver
{
    ...
    function stakeNFT(
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(
                vaults[address(pool.stakingAddress)][tokenId].tokenId == 0,
                "already staked"
            );
            // Transfer staking tokens from the user to the contract
            nft.safeTransferFrom(msg.sender, address(this), tokenId);
            // Update user's staked balance
            vaults[address(pool.stakingAddress)][tokenId] = Stake({
                poolId: _poolId,
                tokenId: tokenId,
    ...
    function _unstakeNFT(
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];
            Stake memory staked = vaults[address(pool.stakingAddress)][tokenId];
            require(staked.owner == account, "not an owner");

            delete vaults[address(pool.stakingAddress)][tokenId];

            // Transfer staking tokens back to the user
            pool.stakingAddress.safeTransferFrom(
                address(this),
                account,
                tokenId
            );
